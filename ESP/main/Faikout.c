...
const struct FanMode fans_5_auto[] = {
   {'A', "auto"},
   {'1', "low"},
   {'2', "lowMedium"},
   {'3', "medium"},
...
const struct FanMode fans_3_auto[] = {
   {'A', "auto"},
   {'1', "low"},
...
jo_comms_alloc (void)
{
   ...
   jo_stringf (j, "protocol", "%s%s%s", b.loopback ? "loopback" : proto_name (),
               !b.loopback && (proto & PROTO_TXINVERT) ? "¬Tx" : "", !b.loopback && (proto & PROTO_RXINVERT) ? "¬Rx" : "");
   return j;
}
         ...
         {                      // Automatic external temperature logic - only really useful if autor/autot set, or Faikout remote
            ...
            if (bletemp && !bletemp->missing && bletemp->faikoutset)
            {
               ...
               if (bletemp->fan && daikin.fan != bletemp->fan - 1 && !(daikin.control_changed & CONTROL_fan))
               {
                  ...
                  if (fan == 7 && (!fan_3_auto () & !fan_5_auto ()))
                     fan = 0;   // No auto
                  if ((fan == 3 || fan == 5) && !fan_5_auto ())
                     fan = 0;   // Only 3 speeds
                  if (fan)
